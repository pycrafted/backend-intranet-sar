...
class VectorSearchService:
    ...
    def search_similar(self, query: str, limit: int = 5, threshold: Optional[float] = None) -> List[DocumentEmbedding]:
        ...
        Intégré avec le cache Redis pour optimiser les performances.
        ...
        try:
            ...
            cached_results = advanced_cache_service.get_cached_search(query, limit, threshold)
            if cached_results:
                logger.info(f"Résultats trouvés dans le cache pour: {query[:50]}...")
                # Convertir les résultats du cache en objets DocumentEmbedding
                results = []
                for cached_doc in cached_results:
                    doc = DocumentEmbedding(
                        id=cached_doc.get('id'),
                        content_text=cached_doc.get('content'),
                        metadata=cached_doc.get('metadata', {}),
                        embedding=[]  # Pas besoin de l'embedding pour le retour
                    )
                    doc.similarity = cached_doc.get('similarity', 0.0)
                    results.append(doc)
            ...
            # Générer l'embedding (vérifier le cache d'embedding aussi)
            ...
            with connection.cursor() as cursor:
                ...
                # Mettre en cache les résultats pour les futures recherches
                if results:
                    cache_data = []
                    for doc in results:
                        cache_data.append({
                    ...
                    advanced_cache_service.cache_search(query, limit, threshold, cache_data)
