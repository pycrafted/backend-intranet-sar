...
@receiver(post_save, sender='accueil.Questionnaire')
def vectorize_questionnaire_on_save(sender, instance, created, **kwargs):
    """
    Vectoriser automatiquement un questionnaire lors de sa création ou modification
    """
    try:
        # Import conditionnel pour éviter les dépendances circulaires
        from rag.services.vectorization_service import VectorizationService
        ...
        # Créer le nouvel embedding
        vectorization_service.vectorize_questionnaire(instance)
        
        action = "créé" if created else "modifié"
        logger.info(f"Questionnaire {instance.id} {action} et vectorisé automatiquement")
        
    except Exception as e:
        logger.error(f"Erreur lors de la vectorisation automatique du questionnaire {instance.id}: {e}")


@receiver(post_delete, sender='accueil.Questionnaire')
def remove_questionnaire_embedding_on_delete(sender, instance, **kwargs):
    """
    Supprimer l'embedding d'un questionnaire lors de sa suppression
    """
    try:
        # Import conditionnel pour éviter les dépendances circulaires
        from rag.models import DocumentEmbedding
        ...
        logger.info(f"Embedding du questionnaire {instance.id} supprimé automatiquement")
        
    except Exception as e:
        logger.error(f"Erreur lors de la suppression de l'embedding du questionnaire {instance.id}: {e}")
